...
class _FindScreenState extends State<FindScreen> {
  ...
  Future<void> _handleSwipe(UserProfile profile, bool isLike) async {
    try {
      final direction = isLike ? 'right' : 'left';
      log(
      ...
      if (success) {
        log('Successfully recorded swipe in backend');
  ...
  Future<void> _rewindLastSwipe() async {
    final success = await ProductionServices.rewindSwipe();
    if (!mounted) return;
  ...
  Future<void> _boostProfile() async {
    final success = await ProductionServices.boostProfile();
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
